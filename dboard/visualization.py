...
def dboard(csv_file, out_dir, bg_range):
    index_html = open('{}/index.html'.format(out_dir), "w")

    entries_df = read_entries_df(csv_file)
    entries_ts = get_traces_ts(entries_df)
    days = get_day_range(entries_ts, weeks_back=3)
    ...
    print("weekly_tir", weekly_tir)

    weekly_days = get_days_in_week(days)
    print("weekly_days", get_days_in_week(days))

    all_data = []

    for day, values in weekly_days.iteritems():
        print(day, values, weekly_tir[day], weekly_avg[day], weekly_est_hba1c[day])
        all_data.append({
            'week_start': day.strftime("%d/%m/%Y"),
            'plots': [day.strftime("%Y/%m/%d/plot.png") for day in values],
            'range_low': bg_range[0],
    ...
    # Blank cells in case first day isn't a Monday (weekday 0)
    if days[0].weekday() != 0:
        print("<tr>", file=index_html)
        print("<td>%s</td>" % days[0].strftime("%d/%m/%Y"), file=index_html)
        for i in range(days[0].weekday()):
            print("<td></td>", file=index_html)
        closedRowTag = False

    for day in days:
        if day.weekday() == 0:
            tir = weekly_tir[day] if day in weekly_tir else 0.0
            ag = weekly_avg[day] if day in weekly_avg else 0.0
            ehba1c = weekly_est_hba1c[day] if day in weekly_est_hba1c else 0.0
            print("<tr>", file=index_html)
            print(
                "<td>%s<br><br>TIR: %.1f%%<br>Avg: %.1f<br>Est A1c: %.1f</td>"
                % (day.strftime("%d/%m/%Y"), tir, ag, ehba1c),
                file=index_html,
            )
            closedRowTag = False
        img_html = daily_plot(days_to_points[day], bg_range, day, out_dir)
        print("<td>%s</td>" % img_html, file=index_html)
        if day.weekday() == 6:
            print("</tr>", file=index_html)
            closedRowTag = True
